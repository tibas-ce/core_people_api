...
RSpec.describe EmployeeProfileBlueprint do
  ...
  describe "visualização mínima" do
    let(:result) do
      JSON.parse(EmployeeProfileBlueprint.render(employee, view: :minimal))
    end

    it "inclui apenas campos básicos" do
      expect(result["position"]).to eq("Desenvolvedor")
      expect(result["department"]).to eq("TI")
  ...
  describe "visualização padrão" do
    subject { JSON.parse(EmployeeProfileBlueprint.render(employee, view: :detail)) }
  ...
  describe "visualização de admin" do
    subject { JSON.parse(EmployeeProfileBlueprint.render(employee, view: :admin)) }
    ...
    it "inclui a data de término, se presente" do
      ...
      result = JSON.parse(EmployeeProfileBlueprint.render(employee, view: :admin))
  ...
  describe "renderizando coleções" do
    ...
    it "renderiza uma array de employees" do
      result = JSON.parse(EmployeeProfileBlueprint.render(employees, view: :minimal))
  ...
  describe "edge cases" do
    it "gerencia employee sem data de nascimento" do
      ...
      result = JSON.parse(EmployeeProfileBlueprint.render(employee, view: :detail))
